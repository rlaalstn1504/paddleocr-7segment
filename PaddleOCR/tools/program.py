...
class ArgsParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("-c", "--config", help="configuration file to use", default="./PaddleOCR/configs/rec/rec_custom.yml")
        # self.add_argument("-c", "--config", help="configuration file to use", default='/PaddleOCR/configs/det/det_custom.yml')
        # self.add_argument("-c", "--config", help="configuration file to use", default='/PaddleOCR/configs/rec/rec_custom_eval.yml')
...
def train(
    ...
    print(config["Architecture"]["model_type"],' start')
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            lr = optimizer.get_lr()
            images = batch[0] 
            # print(images[0].reshape([25,640,640,3]).shape)
            # w_images = wandb.Image(images[0].reshape([25,640,640,3]), caption="Top: Output, Bottom: Input") 
            # wandb.log({"images": w_images}) 
...
def preprocess(is_train=False, task=None):
    if task == "rec_train":
        FLAGS = easydict.EasyDict(
            {
                "config": "PaddleOCR/configs/rec/rec_custom.yml",
                "opt": {},
        ...
        FLAGS = easydict.EasyDict(
            {
                "config": "PaddleOCR/configs/rec/rec_custom_test.yml",
                "opt": {},
                "profiler_options": None,
            }
        ) 
        ...
        FLAGS = easydict.EasyDict(
            {
                "config": "PaddleOCR/configs/det/det_custom_test.yml",
                "opt": {},
                "profiler_options": None,
            }
        )    
    ...
    if (
        ...
        # wandb.run.name = config["Architecture"]["model_type"]
        # wandb.run.save()
